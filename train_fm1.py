...
def main():
    ...
    if args.task != 'herec':
        best = 0
        best_i = 0
        m_episode = 80 if dataset == 'yelp_data' else 150  # 设置测试的最大迭代次数
        for i in range(m_episode):
            print('Current epoch: ', i)
            if i % 1 == 0:
                acc = test_env.test_batch(logger2)  # 计算当前模型在测试集上的准确性
                if acc > best:
                    best = acc
                    best_i = i
            ...
            test_env.train_GNN()  # 继续训练GNN模型
    else:
        test_env.model.steps = 15
        test_env.model.recommend()  # 特殊的推荐方法
