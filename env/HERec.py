...
class HERec:
    def __init__(self, data, metapaths, args, steps, features_dict=None):
        ...
        self.features_dict = features_dict  # 新增的特征字典属性
    ...
    def cal_u(self, i):
        ...
        user_features = self.features_dict.get('user_features', None)
        for k in range(self.user_metapathnum):
            x = self.X[i][k]
            if user_features is not None:
                x += user_features[i]  # 将特征加到嵌入向量中
            ui += self.pu[i][k] * self.sigmod((self.Wu[k].dot(x) + self.bu[k]))
        return self.sigmod(ui)

    def cal_v(self, j):
        vj = np.zeros(self.itemdim)
        item_features = self.features_dict.get('item_features', None)
        for k in range(self.item_metapathnum):
            y = self.Y[j][k]
            if item_features is not None:
                y += item_features[j]  # 将特征加到嵌入向量中
            vj += self.pv[j][k] * self.sigmod((self.Wv[k].dot(y) + self.bv[k]))
